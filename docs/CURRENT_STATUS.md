...
1. 把当前会话骨架从 Demo 推进到真实可复用链路，减少首页 Demo 与微信真实链路的重复代码
2. 细化 LLM 失败提示，把配置、网络、HTTP、解析失败映射为用户可操作建议
3. 先维持 OCR 作为可用兜底，后续再基于真机样本慢慢校准

对应任务建议：

- 真实会话链路抽象：统一上下文整理、候选生成、本地兜底和诊断写入
- LLM 失败建议细化：配置错误、网络失败、HTTP 错误、返回格式错误分别提示
- Gradle 多模块拆分
- Hilt DI 接入
- Navigation 骨架
