...
internal val dateRegex = Regex("""^(\d{1,2}月\d{1,2}日|星期[一二三四五六日天]|昨天|今天|周[一二三四五六日天])(\s*\d{1,2}[:：]\d{2})?$""")
...
internal val embeddedTimeRegex = Regex("""((凌晨|早上|上午|中午|下午|晚上)\s*)?\d{1,2}[:：]\d{2}""")
internal val timeResidualRegex = Regex("""^(凌晨|早上|上午|中午|下午|晚上|早|晚|晚止)$""")
...
internal const val IMAGE_SOURCE_MARKER = "[来自图片]"
internal const val MAX_BUBBLE_OVERLAP_RATIO = 0.02f
internal const val MAX_NEARBY_ROLE_DISTANCE_RATIO = 0.20f
